...
public class CuotaDao {
	...
	public Cuota traerCuota(long idPrestamo, int nroCuota) throws HibernateException {
		...
		try {
			iniciaOperacion();
			objeto = (Cuota) session.createQuery("from Cuota c where c.idPrestamo = " + idPrestamo 
					+ " and c.nroCuota = " + nroCuota).uniqueResult();
		} finally {
			session.close();
		}
		return objeto;
	...
	public List<Cuota> traerCuotas(long idPrestamo) throws HibernateException {
		List<Cuota> lista = null;
		try {
			iniciaOperacion();
			lista = session.createQuery("from Cuota c where c.idPrestamo = " + idPrestamo + "order by c.nroCuota asc").list();
		} finally {
			session.close();
		}
		return lista;
